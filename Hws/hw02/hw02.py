...
def product(n, term):
    ...
    """
    x, total = 1, 1
    while x<= n: # Until x is bigger than n 
...
def accumulate(merger, start, n, term):
    ...
    x, total = 1, start
    while x<=n : 
        x, total = x+1, merger(term(x), total)
    return total 
...
def waves(n):
    ...
    True
    """
    #def crest(prev, curr, next):
        #return prev < curr and next < curr
   # def trough (prev, curr, next):
        #return curr < prev and curr < next
    #def plateau(prev, curr, next):
    ...
    def helper(n, count, prev):
        ...
        if prev > curr:
            if curr == next:
                ...
                # Decrease stay Increase or Decrease Decrease Decrease
                return True and helper(n//10 ,count +1 , n%10)
        elif prev <curr:
            # Increase stay stay or Increase Increase Increase, or Increease stay decrease
                return True and helper(n//10, count +1, n%10)
        else:
            if curr < next:
                # stay 2 increase 
                ...
                # stay stay decrease or stay x3 
                return True and helper(n // 10, count, n % 10)   
...
def count_coins(total):
    ...
    def combinations(amount, coin):
        if amount == 0:
            ...
            current = combinations( amount - coin, coin)
            smaller = combinations (amount, next_smaller_coin(coin))
            return current + smaller
    return combinations(total, 25)
