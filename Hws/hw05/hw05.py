...
def add_d_leaves(t, v):
    ...
    3
      ...
      2
        ...
        10
    """
    def leaves_helper(t,v, d=0):
        for b in t.branches:
            leaves_helper(b, v, d +1)
        for _ in range(d):
...
def store_digits(n):
    ...
    def reverse(link):
        if link == Link.empty:
            ...
            Links = Link (link.first, Link.empty)
            while link.rest != Link.empty:
                link = link.rest
                Links = Link(link.first, Links)
            return Links 

    def store_digits_order(n):
        if n//10 == 0:
            return Link(n)
        else:
            return Link(n %10, store_digits_order(n //10))
    return reverse(store_digits_order(n))
