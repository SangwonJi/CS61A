...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        temp = scheme_eval(expressions.rest.first, env)
        env.define(variable_name, temp) #!!!
        ...
        while formal_curr is not nil:
            if not scheme_symbolp(formal_curr.first):  

                raise SchemeError(f'invalid formal parameter: {formal_curr.first}')
            formal_curr = formal_curr.rest

        body = expressions.rest
        lambda_proc = LambdaProcedure(formals, body, env)########AGAIN
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        if not scheme_listp(binding) or len(binding) != 2 or not scheme_symbolp(binding.first):
            raise SchemeError("SchemeError binding: {}".format(str(binding)))
        
        if binding.first in check_name:
            raise SchemeError("SchemeError binding: {}".format(str(binding.first)))

        check_name.add(binding.first)
...
def do_mu_form(expressions, env):
    ...
    validate_formals(formals)
    # BEGIN PROBLEM 11
    return MuProcedure(formals, expressions.rest.first)
