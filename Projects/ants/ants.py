...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None: #Empty
            place.ant = self

        #Existing ant container and can contain new ant
        elif place.ant.is_container and place.ant.can_contain(self): 

            place.ant.store_ant(self)
        # New ant is container ant and can contain exsisting ant
        elif self.is_container and self.can_contain(place.ant): 
            old_ant = place.ant
            place.ant = self
            self.store_ant(old_ant)
    ...
    def double(self):
        ...
        if not self.doubled:  
            self.damage *= 2
            self.doubled = True # change to True is doubled
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        place = self.place
        while place:
            if (place.is_hive!=True) and (self.lower_bound<= distance <=self.upper_bound):
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        total_damage = amount # Total damage to be refelcted
        if self.health - amount <= 0:  # when fire ant die
            total_damage += self.damage
            
        for bee in list(self.place.bees):  #Apply refelctive damage
            bee.reduce_health(total_damage)
        super().reduce_health(amount) #reduction and removal 
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate): ###########
        if self.turns_to_chew > 0:
            ...
            if bees_in_place:
                eaten_bee = random.choice(bees_in_place)
...
# END Problem 12
    ...
    def __init__(self, health=1):
        """Create an Ant with a HEALTH quantity."""
        super().__init__(health)
        self.boosted_ants =[] # Damage *2 tracker
    ...
    def action(self, gamestate):
        ...
        while curr_place:
            if isinstance(curr_place.ant, ContainerAnt) and curr_place.ant.ant_contained == self:
             pass

            elif curr_place.ant and (curr_place.ant not in self.boosted_ants) and not isinstance(curr_place.ant, QueenAnt):
                curr_place.ant.double() 
            self.boosted_ants.append(curr_place.ant)

            if (isinstance(curr_place.ant, ContainerAnt) and curr_place.ant.ant_contained 
                and (curr_place.ant.ant_contained not in self.boosted_ants) 
                and (curr_place.ant.ant_contained != self)
                ):

                curr_place.ant.ant_contained.double()
                self.boosted_ants.append(curr_place.ant.ant_contained)
       ...
       # Track that we have boosted this ant
    ...
    def reduce_health(self, amount):
        ...
        self.health -= amount
        if self.health <= 0 and isinstance(self, QueenAnt):  # If the ant is a QueenAnt
            raise AntsLoseException("Ants have lost!")
        # END Problem 12
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if not hasattr(target, 'slowed'):
            ...
            target.action = self.slowed_action_outer(target)
            target.slowed_count = 3
        else:
            target.slowed_count = 3  
    def slowed_action_outer(self, target):
        def slowed_action(gamestate):
             if gamestate.time % 2 == 0:
                target.original_action(gamestate)
                target.slowed_count -= 1
             if target.slowed_count <= 0:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        if target is None:
            return
        if not hasattr(target, "has_been_scared") or not target.has_been_scared:
            target.scare(2)
        # END Problem EC 2
...
class Bee(Insect):
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC 2
        if not hasattr(self, "has_been_scared"):
            self.has_been_scared = True
            self.original_move_to = self.move_to  

        def scared_move_to(place):
            if hasattr(self, 'scared_turns') and (self.scared_turns > 0):
                self.scared_turns -= 1
                if self.place.entrance:  
                    self.original_move_to(self.place.entrance)
            else:
                self.move_to = self.original_move_to
                self.move_to(place)
        self.move_to = scared_move_to #backwards
